...
def process():
    ...
    # Buscar columna de aportes
    celda_aporte = 0
    total_aporte = 0
    for c in wSheet.iter_cols(min_col=1, values_only = True):
        if celda_aporte >= celda_bono:
            break
        if mensaje_aporte.lower() not in c[0].lower():
            celda_aporte += 1
        else:
            break
    ...
    # Buscar columna de hecho 
    celda_hecho = 0
    total_hecho = 0
    for c in wSheet.iter_cols(min_col=1, values_only = True):
        if celda_hecho >= celda_bono:
            break
        if mensaje_hecho.lower() not in c[0].lower():
            celda_hecho += 1
        else:
            break
    ...
    for r in wSheet.iter_rows(min_row=2, values_only=True):
            ...
            for c in range(1, len(r)):
                ...
                if wSheet.cell(row=1, column=c+1).value and ("\"1\"" not in wSheet.cell(row=1, column=c+1).value) and ("$" in wSheet.cell(row=1, column=c+1).value or "=" in wSheet.cell(row=1, column=c+1).value):
                    if 'Registrá' in wSheet.cell(row=1, column=c+1).value:
                        break
                    elif mensaje_aporte in wSheet.cell(row=1, column=c+1).value:
                        break
                    elif r[c] != None:
                        try:
        ...
        if celda_aporte > 0 and r[celda_aporte] != None:
            compradores[nombre].aporte = int(r[celda_aporte])
            total_aporte += int(r[celda_aporte])
        
        if celda_hecho > 0 and r[celda_hecho] != None:
            compradores[nombre].hecho = int(r[celda_hecho])
            total_hecho += int(r[celda_hecho])


    #Imprimir vendedores
    print("Produciendo salida")
    print("Imprimiendo productores")
    sheet_vendedores = "Detalle por Productor"
    if sheet_vendedores not in inFile.get_sheet_names():
        ...
        for v in sorted(vendedores.keys()):
            ...
            for p in vendedores[v].products:
                ...
                vend_sheet.cell(row=row_idx, column=2).number_format = '0.00'
                vend_sheet.cell(row=row_idx, column=2).value = vendedores[v].prices[p]/1.08 * vendedores[v].vendidos[p]
                #vend_sheet.cell(row=row_idx, column=2).value = "{:.2f}".format(vendedores[v].prices[p]/1.05 * vendedores[v].vendidos[p]).replace('.', ',')
            ...
            vend_sheet.cell(row=row_idx, column=3).number_format = '0.00'
        ...
        warningMessage = "Ya existe la hoja "+ sheet_vendedores + ", desea sobreescribirla?"
    ...
    sheet_compradores = "Detalle por Encargo"
    ...
    if sheet_compradores not in inFile.get_sheet_names():
        ...
        for c in compradores.values():
            ...
            if c.aporte != 0:
                comp_sheet.cell(row=row_idx, column=2).value = "Aporte"
                comp_sheet.cell(row=row_idx, column=3).value = c.aporte
                row_idx += 1
                total_c += c.aporte
            
            if c.hecho != 0:
                comp_sheet.cell(row=row_idx, column=2).value = "Hecho Solidario"
                comp_sheet.cell(row=row_idx, column=3).value = c.hecho
                row_idx += 1
                total_c += c.hecho
        ...
        #if user_option == "Si" or user_option == "si":
        warningMessage = "Ya existe la hoja " + sheet_compradores + ", desea sobreescribirla?"
        user_option = messagebox.askyesno(title = "Hoja existente", message=warningMessage)
        if user_option:
            comp_sheet = inFile.get_sheet_by_name(sheet_compradores)
    ...
    #Imprimir compradores alfabeticamente
    sheet_compradores_alf = "Detalle por Encargo ordenado"
    print("Imprimiendo compradores ordenado alfabeticamente")
    if sheet_compradores_alf not in inFile.get_sheet_names():
        inFile.create_sheet(sheet_compradores_alf)
    else:
        comp_sheet = inFile.get_sheet_by_name(sheet_compradores_alf)
        inFile.remove_sheet(comp_sheet)
        inFile.create_sheet(sheet_compradores_alf)
    comp_sheet = inFile.get_sheet_by_name(sheet_compradores_alf)
    row_idx = 1
    ...
    for c in sorted(compradores.keys()):
        comp_sheet.cell(row=row_idx, column=1).value = compradores[c].name
        comp_sheet.cell(row=row_idx, column=1).fill = PatternFill("solid", fgColor=bg_color_main)
        comp_sheet.cell(row=row_idx, column=1).font = Font(bold=True)
        #comp_sheet.cell(row=row_idx, column=2).value = "Cantidad"
        comp_sheet.cell(row=row_idx, column=2).fill = PatternFill("solid", fgColor=bg_color_main)
        ...
        total_c = 0
        for p in sorted(compradores[c].products, key=lambda x: x[1]):
            comp_sheet.cell(row=row_idx, column=1).value = p[1]
            comp_sheet.cell(row=row_idx, column=2).value = p[2]
            nombre_v = p[1].split('.')[0] if '.' in p[1].split('[')[0] else p[1].split('[')[0]
            p_precio = vendedores[nombre_v].prices[p[1]]
            ...
            row_idx += 1
            total_c += int(p[2]*p_precio)
        total_c += compradores[c].bono

        if compradores[c].bono != 0:
            comp_sheet.cell(row=row_idx, column=2).value = "Bono"
            comp_sheet.cell(row=row_idx, column=3).value = compradores[c].bono
            row_idx += 1

        if compradores[c].aporte != 0:
            comp_sheet.cell(row=row_idx, column=2).value = "Aporte"
            comp_sheet.cell(row=row_idx, column=3).value = compradores[c].aporte
            row_idx += 1
            total_c += compradores[c].aporte
        
        if compradores[c].hecho != 0:
            comp_sheet.cell(row=row_idx, column=2).value = "Hecho Solidario"
            comp_sheet.cell(row=row_idx, column=3).value = compradores[c].hecho
            row_idx += 1
            total_c += compradores[c].hecho
    ...
    if(isFinalProcess): #cierre de operatoria
        ...
        for c in compradores.values():
            ...
            comp_sheet_final.cell(row=row_idx, column=2).number_format = '0.00'
            comp_sheet_final.cell(row=row_idx, column=2).value = c.total
            row_idx+=1
        ...
        if total_aporte > 0:
            row_idx+=1
            row_idx+=1
            comp_sheet_final.cell(row=row_idx, column=1).value = "Total aportes"
            comp_sheet_final.cell(row=row_idx, column=2).number_format = '0.00'
            comp_sheet_final.cell(row=row_idx, column=2).value = total_aporte

        print("Realizando resumen final de productores")
        sheet_vendedores_final = "Resumen por Productores"
        ...
        for v in sorted(vendedores.keys()):
            ...
            vend_sheet_final.cell(row=row_idx, column=2).number_format = '0.00'
            vend_sheet_final.cell(row=row_idx, column=2).value = vendedores[v].total
            row_idx+=1
